// SHA-256 from Python’s hashlib module
...
//for db of users.
typedef struct {
    Block *head;    // linked list of blocks, 
...
//DJB2 algorithm...return the index of hash table where we want to store user. 
int hashFunction(char *str) {
    unsigned long hash = 5381;  //.. large prime number (5381) — a commonly used seed in djb2.
    int c;
    while ((c = *str++)) //..Loops through each character in the string (until null-terminator).

        hash = hash * 33 + c;  //..Multiplies the current hash by 33 and adds the character’s ASCII value.

    return hash % TABLE_SIZE;
}

//DJB2 algorithm - noncryptographic
...
//initilising the user database to NULL
...
//using linked list stored users.

/*✅ Efficient Lookup: Instead of searching the entire list of users, you can hash the account number and jump directly to the right bucket.

🔁 Collision Handling: Multiple users may hash to the same index. You handle this by chaining them using a linked list (via the next pointer).

🧩 Dual Purpose: Notice that you’re maintaining two structures:

A regular linked list (db->users) — possibly for sequential operations like displaying all users.

A hash table (db->hashTable) — for fast access by account number.*/
...
User *createUser(BankDatabase *db, const char *name, const char *mobile, const char *password, float initialDeposit) {
    ...
    newUser->next = db->hashTable[index];  //Points the new user’s next pointer to the current head of the list at that index in the hash table. This enables chaining — a method to handle collisions.
    db->hashTable[index] = newUser;  //storing user to db->hashtable[index].
...
User* findUser(BankDatabase *db, const char* accountNumber) {
    /*The hash function helps locate the exact bucket where a user might be.
It dramatically reduces search time from O(n) to O(1) on average.*/
...
void loadUsersFromCSV(BankDatabase *db, char *filename) {
    ...
    if (!file) {
        ...
        fprintf(file, "AccountNumber,Name,Mobile,Password,Balance\n");
        fclose(file);
        printf("New users file created: %s\n", filename);
        return;
    }
...
//has 2 part 1 for deposit and withdraw.
void transaction(BankDatabase *db, const char* accountNumber, float amount, int type) {
    ...
    addBlock(data); //saving transaction in blockchain of transaction.
}
...
void menu() {
    BankDatabase db; //is linkedlist with linking done with hash function. database of users.
    //Initializes hash table and loads saved user/transaction data from files.
    initBankDatabase(&db);
    loadUsersFromCSV(&db, "users.csv");
